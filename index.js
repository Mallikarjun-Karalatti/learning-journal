const viewMoreEl = document.getElementById('view-more') 
const selectedArticleEl = document.getElementById('selected-article')
const articleContainer = document.querySelector('.article-container')
const articles = document.querySelectorAll('.article')
const heroSection = document.getElementById('hero-section')
const homeLiEl = document.getElementById('home-li')
const aboutMeLiEl = document.getElementById('about-me-li')
const aboutMeEl = document.getElementById('about-me')

let isNewArticlesAppended = false
selectedArticleEl.style.display = 'none'
aboutMeEl.style.display = 'none'
    
viewMoreEl.addEventListener('click', () => {
    toggleArticles()
})

articles.forEach( article => {
    article.addEventListener('click', () => {
        heroSection.style.display = 'none';
        aboutMeEl.style.display = 'none';
        addSelectedArtilce()
    })
})

homeLiEl.addEventListener('click', () => {
    heroSection.style.display = 'block'
    selectedArticleEl.style.display = 'none'
    aboutMeEl.style.display = 'none';
})

aboutMeLiEl.addEventListener('click', () => {
    heroSection.style.display = 'none'
    selectedArticleEl.style.display = 'none'
    addAboutMeEl()
})

function toggleArticles(){
    const existingExtras = document.querySelectorAll('.article.extra')

    if (!isNewArticlesAppended) {
        // create and append extra articles directly as siblings
        const extra1 = document.createElement('div')
        extra1.classList.add('article','extra')
        extra1.innerHTML = `
            <img src="/images/colorful-pic.png" alt="a colorful background"/>
            <p class="txt-date">JULY 23, 2022</p>
            <h3>Blog four</h3>
            <p>I'm excited to start a new learning journey as a Scrimba Bootcamp student! After several months of learning in the Frontend Developer Career Path.</p>
        `

        const extra2 = document.createElement('div')
        extra2.classList.add('article','extra')
        extra2.innerHTML = `
            <img src="/images/lady-walk.png" alt="a lady walking"/>
            <p class="txt-date">JULY 23, 2022</p>
            <h3>Blog five</h3>
            <p>I'm excited to start a new learning journey as a Scrimba Bootcamp student! After several months of learning in the Frontend Developer Career Path.</p>
        `

        const extra3 = document.createElement('div')
        extra3.classList.add('article','extra')
        extra3.innerHTML = `
            <img src="/images/react-code.png" alt="a laptop showing react code"/>
            <p class="txt-date">JULY 23, 2022</p>
            <h3>Blog six</h3>
            <p>I'm excited to start a new learning journey as a Scrimba Bootcamp student! After several months of learning in the Frontend Developer Career Path.</p>
        `

        articleContainer.append(extra1, extra2, extra3)
        isNewArticlesAppended = true
        viewMoreEl.textContent = "View Less"
    } else {
        // remove all extra articles
        existingExtras.forEach(el => el.remove())
        isNewArticlesAppended = false
        viewMoreEl.textContent = "View More"
    }
}

function addSelectedArtilce() {
        
        selectedArticleEl.innerHTML = `
            <p class="fs-small">APRIL 14, 2026</p>
            
            <h2>My new journey as a bootcamp student.</h2>
            
            <p>After several months of learning in the Frontend Developer Career Path, I've made the big jump over to the Bootcamp to get expert code reviews of my Solo Projects projects and meet like-minded peers.</p>
            <img src="/images/article-image-hero.png" alt="a loptop opened and on. It's in a dark room with round table lamp"/>
            
            <h3>How I stay committed to learning</h3>

            <p>I like to think of myself as a lifelong learner. I used to spend hours and hours learning, then try to create simple projects using what I learned or work new techniques into existing projects.</p>
            
            <p>While that was fun, I felt like it would be helpful to share what I was learning and most things about my journey with the world.</p>
            
            <h3>How I got started</h3>
            
            <p>I started simple and gradually grew my learning journal site. I would take notes about what I was learning. After each learning session, I'd use my notes to not only reflect on what I learned but also write short summaries of what I learned using my own words.</p>
            
            <p>That helped me grok what I was learning, and I realized that posting my learning summaries was also helping others learn and stay motivated.</p>        
                        
        `
        selectedArticleEl.style.display = 'flex'
}


function addAboutMeEl(){
    aboutMeEl.innerHTML = `
        <div class="my-info">
            <img src="/images/My_Gibli_Pic.jpeg" alt="Arjun's gibli anime pic with green background"/>
            <div class="my-info-text">
                <h2>Hi there! My name is Arjun and welcome to my learning journal.</h2>
                <p>After several months of learning in the Frontend Developer Career Path, I've made the big jump over to the Bootcamp to get expert code reviews of my Solo Projects projects and meet like-minded peers.</p>
            </div>
        </div>
       <div class="about-me-article">
            <h3>How I stay committed to learning</h3>
    
            <p>I like to think of myself as a lifelong learner. I used to spend hours and hours learning, then try to create simple projects using what I learned or work new techniques into existing projects.</p>
            
            <p>While that was fun, I felt like it would be helpful to share what I was learning and most things about my journey with the world.</p>
            
            <h3>How I got started</h3>
            
            <p>I started simple and gradually grew my learning journal site. I would take notes about what I was learning. After each learning session, I'd use my notes to not only reflect on what I learned but also write short summaries of what I learned using my own words.</p>
            
            <p>That helped me grok what I was learning, and I realized that posting my learning summaries was also helping others learn and stay motivated.</p>
        </div>
        `
        aboutMeEl.style.display = 'block'
}
